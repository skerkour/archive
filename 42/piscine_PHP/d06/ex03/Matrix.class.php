...
class Matrix {
	...
	function __construct(array $kwargs) {
		if ($kwargs["preset"] === self::SCALE) {
		...
		else if (array_key_exists("angle", $kwargs)
				&& ($kwargs["preset"] === self::RX
				|| $kwargs["preset"] === self::RY
				|| $kwargs["preset"] === self::RZ )) {
			if ($kwargs["preset"] === self::RX)
				$this->_iRx($kwargs["angle"]);
			if ($kwargs["preset"] === self::RY)
				$this->_iRy($kwargs["angle"]);
			if ($kwargs["preset"] === self::RZ)
				$this->_iRz($kwargs["angle"]);
		}
			...
			$this->_projMatrix($kwargs["fov"], $kwargs["ratio"],
				$kwargs["near"], $kwargs["far"]);
	...
	private function _projMatrix($fov, $ratio, $near, $far) {
		$this->_matrix = array(
			...
			array(0, 0, -$far * (1 / ($far - $near)), -1 - ($far * $near)
				* (1 / ($far - $near))),
			array(0, 0, -1, 0));
		if (self::$verbose)
	...
	public function mult(Matrix $rhs) {
		...
		while ($i < 4) {
			...
			while ($j < 4) {
				...
				$sum = 0;
				while (++$jj < 4)
					$sum += $this->_matrix[$i][$jj] * $rhs->_matrix[$jj][$j];
				$res->_matrix[$i][$j] = $sum;
				$j++;
			}
			$i++;
