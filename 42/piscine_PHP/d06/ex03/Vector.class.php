...
Class Vector {
	...
	function __toString() {
		return sprintf("Vector( x:%3.2f, y:%3.2f, z:%3.2f, w:%3.2f )", $this->_x,
	...
	public function normalize() {
		$m = $this->magnitude();
		if ($m)
			return new Vector(array('x' => $this->_x / $m,  'y' => $this->_y / $m,
				'z' => $this->_z / $m));
	}

	public function add(Vector $rhs) {
		return new Vector(array('x' => $this->_x + $rhs->getX(),
	...
	public function opposite() {
		return new Vector(array('x' => -$this->_x, 'y' => -$this->_y,
			'z' => -$this->_z));
	}

	public function scalarProduct($k) {
		return new Vector(array('x' => $this->_x * $k,  'y' => $this->_y * $k,
			'z' => $this->_z * $k));
	}

	public function dotProduct(Vector $rhs) {
		return ($this->_x * $rhs->getX() + $this->_y * $rhs->getY() + $this->_z
			* $rhs->getZ());
	}
	
	public function cos(Vector $rhs) {
		return (($this->_x * $rhs->getX() + $this->_y * $rhs->getY() + $this->_z
			* $rhs->getZ()) / sqrt($this->_length() * $rhs->_length()));
	}
