...
func TestDoJSON(t *testing.T) {
	...
	err := client.Do(ctx, &Request{q: "query {}"}, &responseData)
	is.NoErr(err)
...
func TestQueryJSON(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		b, err := ioutil.ReadAll(r.Body)
		is.NoErr(err)
		is.Equal(string(b), `{"query":"query {}","variables":{"username":"z0mbie42"}}`+"\n")
	...
	req.Var("username", "z0mbie42")

	// check variables
	is.True(req != nil)
	is.Equal(req.vars["username"], "z0mbie42")
