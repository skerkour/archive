...
//! into SANE documents (as strings). Note that some top-level functions here
//! are also provided at the top of the crate.
//!
//! Note that the SANE format has a restriction that if a table itself contains
...
///     };
///
///     let sane = sane::to_string(&config).unwrap();
///     println!("{}", sane)
/// }
/// ```
...
pub enum Error {
    ...
    /// supported.
    ///
    /// Currently the SANE format does not support serializing types such as
    /// enums, tuples and tuple structs.
    UnsupportedType,

    /// The key of all SANE maps must be strings, but serialization was
    ...
    /// Lists in SANE must have a homogenous type, but a heterogeneous array
    /// was emitted.
    ListMixedType,

    /// All values in a SANE table must be emitted before further tables are
...
struct Settings {
    array: Option<ListSettings>,
    string: Option<StringSettings>,
}

/// Serialization implementation for SANE.
///
/// This structure implements serialization support for SANE to serialize an
/// arbitrary type to SANE. Note that the SANE format does not support all
...
impl<'a> Serializer<'a> {
    /// Creates a new serializer which will emit SANE into the buffer provided.
    ///
    ...
    pub fn pretty(dst: &'a mut String) -> Serializer<'a> {
        Serializer {
            ...
            settings: Rc::new(Settings {
                array: Some(ListSettings::pretty()),
    ...
    pub fn pretty_array_indent(&mut self, value: usize) -> &mut Self {
        ...
        if use_default {
            let mut array = ListSettings::pretty();
            array.indent = value;
    ...
    fn array_type(&mut self, type_: &'static str) -> Result<(), Error> {
        let prev = match self.state {
            State::List { type_, .. } => type_,
            _ => return Ok(()),
        };
        if let Some(prev) = prev.get() {
            if prev != type_ {
                return Err(Error::ListMixedType);
            }
        } else {
            prev.set(Some(type_));
        }
    ...
    fn emit_table_header(&mut self, state: &State) -> Result<(), Error> {
        ...
        // headers for them.
        let mut p = state;
        if let State::List { first, parent, .. } = *state {
            if first.get() {
                p = parent;
            }
        }
    ...
    fn emit_key_part(&mut self, key: &State) -> Result<bool, Error> {
        match *key {
            State::List { parent, .. } => self.emit_key_part(parent),
            State::End => Ok(true),
            State::Table {
                key,
                parent,
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::ListMixedType => "arrays cannot have mixed types".fmt(f),
...
impl error::Error for Error {
    fn description(&self) -> &str {
        match *self {
            ...
            Error::ListMixedType => "arrays cannot have mixed types",
...
/// SANE.
///
/// SANE carries the restriction that keys in a table must be serialized last if
    ...
    S: ser::Serializer,
{
    ...
    for (k, v) in data {
        if let Category::List = v.serialize(Categorize::new())? {
            map.serialize_entry(&k, &v)?;
        }
    }
    for (k, v) in data {
