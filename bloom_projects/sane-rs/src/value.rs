...
/// Type representing a SANE map, payload of the `Value::Map` variant.
/// Represents a SANE key/value type.
/// It is backed by a BTreeMap
pub type Map = BTreeMap<String, Value>;

impl Value {
    /// Convert a `T` into `sane::Value` which is an enum that can represent
    /// any valid SANE data.
    ///
    ...
    /// Interpret a `sane::Value` as an instance of type `T`.
    ///
    /// This conversion can fail if the structure of the `Value` does not match the
    /// structure expected by `T`, for example if `T` is a struct type but the
    /// `Value` contains something other than a SANE map. It can also fail if the
    ...
    /// Index into a SANE list or map. A string index can be used to access a
    /// value in a map, and a usize index can be used to access an element of an
    /// list.
    ///
    /// Returns `None` if the type of `self` does not match the type of the
    /// index, for example if the index is a string and `self` is an list or a
    ...
    /// Mutably index into a SANE list or map. A string index can be used to
    ...
    ///
    /// Note that a parsed SANE value will only contain ISO 8601 dates. An
    /// example date is:
    ///
    /// ```notrust
...
impl<V: Into<Value>> From<Vec<V>> for Value {
    fn from(val: Vec<V>) -> Value {
        Value::List(val.into_iter().map(|v| v.into()).collect())
    }
}

impl<S: Into<String>, V: Into<Value>> From<BTreeMap<S, V>> for Value {
    fn from(val: BTreeMap<S, V>) -> Value {
        let map = val.into_iter().map(|(s, v)| (s.into(), v.into())).collect();
...
///
/// Currently this is implemented for `usize` to index lists and `str` to index
/// maps.
///
...
impl Index for usize {
    ...
    fn index_mut<'a>(&self, val: &'a mut Value) -> Option<&'a mut Value> {
        match *val {
            Value::List(ref mut a) => a.get_mut(*self),
            _ => None,
        }
    }
}
...
impl Index for str {
    ...
    fn index_mut<'a>(&self, val: &'a mut Value) -> Option<&'a mut Value> {
        match *val {
            Value::Map(ref mut a) => a.get_mut(self),
            _ => None,
        }
    }
}
        ...
        S: ser::Serializer,
    {
        ...
        match *self {
            ...
            Value::Map(ref t) => {
                ...
                // Be sure to visit non-maps first (and also non
                // list-of-maps) as all keys must be emitted first.
                for (k, v) in t {
                    if !v.is_map() && !v.is_list()
                        || (v
                            .as_list()
                            .map(|a| !a.iter().any(|v| v.is_map()))
                            .unwrap_or(false))
                    {
        ...
        D: de::Deserializer<'de>,
    {
        ...
        impl<'de> de::Visitor<'de> for ValueVisitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("any valid SANE value")
            }
                ...
                V: de::MapAccess<'de>,
            {
                ...
                let datetime = visitor.next_key_seed(DatetimeOrMap { key: &mut key })?;
                match datetime {
                    Some(true) => {
...
struct DatetimeOrMap<'a> {
    key: &'a mut String,
}

impl<'a, 'de> de::DeserializeSeed<'de> for DatetimeOrMap<'a> {
    type Value = bool;
...
impl<'a, 'de> de::Visitor<'de> for DatetimeOrMap<'a> {
    type Value = bool;
