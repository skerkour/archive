...
parse_args() {
  #BINDIR is $HOME/.rocket unless set be ENV
  # over-ridden by flag below

  BINDIR=${BINDIR:-$HOME/.rocket}
...
execute() {
  ...
  hash_sha512_verify "${TMPDIR}/${TARBALL}" "${TMPDIR}/${CHECKSUM}"
  echo "$PREFIX: checksums successfully verified"
...
hash_sha512() {
  ...
  if is_command gsha512sum; then
    ...
    echo "hash_sha512: unable to find command to compute sha-512 hash"
    return 1
  fi
}
hash_sha512_verify() {
  ...
  checksums=$2
  if [ -z "$checksums" ]; then
    echo "hash_sha512_verify: checksum file not specified in arg2"
    return 1
  fi
  BASENAME=${TARGET##*/}
  want=$(grep "${BASENAME}" "${checksums}" 2>/dev/null | tr '\t' ' ' | cut -d ' ' -f 1)
  if [ -z "$want" ]; then
    echo "hash_sha512_verify: unable to find checksum for '${TARGET}' in '${checksums}'"
    return 1
  fi
  got=$(hash_sha512 "$TARGET")
  if [ "$want" != "$got" ]; then
    echo "hash_sha512_verify: checksum for '$TARGET' did not verify ${want} vs $got"
    return 1
  fi
}
cat /dev/null <<EOF
...
CHECKSUM=${BINARY}_${VERSION}_sha512sums.txt
