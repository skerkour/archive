...
//!
//! This module contains all the Serde support for deserializing SANE documents
...
/// deserialize `T` from the SANE document provided.
...
///
/// This function will attempt to interpret `s` as a SANE document and
...
///     "#).unwrap();
///
///     assert_eq!(config.title, "SANE Example");
...
enum ErrorKind {
    ...
    /// An list was decoded but the types inside of it were mixed, which is
    /// disallowed by SANE.
    MixedListType,
    ...
    /// A previously defined table was redefined as an list.
    RedefineAsList,
...
impl<'de, 'b> de::MapAccess<'de> for MapVisitor<'de, 'b> {
        ...
        K: de::DeserializeSeed<'de>,
    {
        ...
        loop {
            ...
            // then this is an error in the sane format
            ...
            //      [[foo]]
            if table.list {
                let kind = ErrorKind::RedefineAsList;
        ...
        V: de::DeserializeSeed<'de>,
    {
        ...
        let list =
            self.tables[self.cur].list && self.depth == self.tables[self.cur].header.len() - 1;
        self.cur += 1;
        let res = seed.deserialize(MapVisitor {
            values: Vec::new().into_iter(),
            next_value: None,
            depth: self.depth + if list { 0 } else { 1 },
...
impl<'de, 'b> de::SeqAccess<'de> for MapVisitor<'de, 'b> {
        ...
        K: de::DeserializeSeed<'de>,
    {
        ...
        let next = self.tables[..self.max]
            ...
            .find(|&(_, table)| table.list && table.header == self.tables[self.cur_parent].header)
            .map(|p| p.0)
            .unwrap_or(self.max);
...
impl<'a> Deserializer<'a> {
    ...
    /// Historical versions of sane-rs accidentally allowed a newline after a
    /// table definition, but the SANE spec requires a newline after a table
    /// definition header.
    ///
    /// This option can be set to `false` (the default is `true`) to emulate
    /// this behavior for backwards compatibility with older sane-rs versions.
    pub fn set_require_newline_after_table(&mut self, require: bool) {
        self.require_newline_after_table = require;
    }

    /// Historical versions of sane-rs accidentally allowed a duplicate table
    /// header after a longer table header was previously defined. This is
    /// invalid according to the SANE spec, however.
    ///
    ...
    fn table_header(&mut self) -> Result<Line<'a>, Error> {
        ...
        let list = self.eat(Token::LeftBracket)?;
        let ret = Header::new(self.tokens.clone(), list, self.require_newline_after_table);
    ...
    fn value(&mut self) -> Result<Value<'a>, Error> {
        ...
        let value = match self.next()? {
            ...
            Some((Span { start, .. }, Token::LeftBracket)) => {
                self.list().map(|(Span { end, .. }, list)| Value {
                    e: E::List(list),
                    start: start,
                    end: end,
                })?
    ...
    // TODO(#140): shouldn't buffer up this entire list in memory, it'd be
    // great to defer parsing everything until later.
    fn list(&mut self) -> Result<(Span, Vec<Value<'a>>), Error> {
        let mut ret = Vec::new();
        ...
        loop {
            ...
            if let Some(last) = ret.last() {
                if !value.same_type(last) {
                    return Err(self.error(at, ErrorKind::MixedListType));
                }
            }
            ret.push(value);
            intermediate(self)?;
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.inner.kind {
            ...
            ErrorKind::MixedListType => "mixed types in an list".fmt(f)?,
            ErrorKind::DuplicateTable(ref s) => {
                write!(f, "redefinition of table `{}`", s)?;
            }
            ErrorKind::RedefineAsList => "table redefined as list".fmt(f)?,
...
impl error::Error for Error {
    fn description(&self) -> &str {
        match self.inner.kind {
            ...
            ErrorKind::MixedListType => "mixed types in an list",
            ErrorKind::DuplicateTable(_) => "duplicate table",
            ErrorKind::RedefineAsList => "table redefined as list",
...
impl<'a> Header<'a> {
    fn new(tokens: Tokenizer<'a>, list: bool, require_newline_after_table: bool) -> Header<'a> {
        Header {
            first: true,
            list: list,
            tokens: tokens,
